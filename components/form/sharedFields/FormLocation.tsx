    ...
    <FormField
      ...
      render={({ field }) => (
        <FormItem className="flex flex-col">
          ...
          <FormControl>
            <Input
              className="ring-transparent dark:bg-gray-800 placeholder:text-gray-400 sm:text-[14px] text-[12px] dark:text-white-50 text-gray-950"
              form={form}
              ...
              <PopoverContent className="w-[410px] z-10 bg-white-50 p-2 dark:bg-gray-800 rounded-md dark:border-none border border-1 shadow-md  ">
                <ul>
                  {placePredictions.map((prediction) => (
                    <li key={prediction.place_id} className="cursor-pointer hover:!text-primary/80 dark:text-white-50" onClick={() => handleClick(prediction.description)}>
