      ...
      render={({ field }) => (
        <FormItem className={`flex flex-col ${leftImageUrl === true ? "w-1/2" : ""}`}>
          ...
          <Select onValueChange={field.onChange} defaultValue={field.value}>
            <FormControl>
              <SelectTrigger className={cn("text-left  h-[46px] sm:h-[56px] bg-white-200 dark:bg-gray-800 text-[14px] focus:ring-transparent border-none data-[placeholder]:!text-gray-400 data-[placeholder]:font-normal data-[placeholder]:font-plusJakartaSans", selectTriggerClassName)}>
                <SelectValue placeholder={placeholder} />
              </SelectTrigger>
            </FormControl>
            <SelectContent className="bg-white-50 dark:bg-gray-800 dark:text-white-50 border-none">
              {options.map((option) => (
                <SelectItem key={option.value} value={option.value} className="hover:text-primary  cursor-pointer">
                  {option.label}
                </SelectItem>
              ))}
            </SelectContent>
